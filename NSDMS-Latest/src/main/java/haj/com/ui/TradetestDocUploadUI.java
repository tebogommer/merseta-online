...
@ManagedBean(name = "tradetestDocUploadUI")
@ViewScoped
public class TradetestDocUploadUI extends AbstractUI {
	...
	private void runInit() throws Exception {
		if (getSessionUI().getTask() != null && getSessionUI().getTask().getWorkflowProcess() == ConfigDocProcessEnum.TradeTestDocumentReUpload) {
	...
	private void populateRejectionReasons() throws Exception{
		rejectionReasons = rejectReasonsService.locateReasonsSelectedByTargetKeyClassAndProcess(companyLearnersTradeTest.getId(), companyLearnersTradeTest.getClass().getName(), ConfigDocProcessEnum.TradeTestDocumentReUpload);
	}
	...
	public void completeWorkflow(){
		try {
			companyLearnersTradeTestService.completeTradeTestDocUploadTask(companyLearnersTradeTest, getSessionUI().getActiveUser(), getSessionUI().getTask());
			ajaxRedirectToDashboard();
		} catch (Exception e) {
			addErrorMessage(e.getMessage(), e);
		}
