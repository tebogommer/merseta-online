 ...
 */
@ManagedBean(name = "qmrSkillsProgrammeUnemployedEnteredUI")
@ViewScoped
public class QmrSkillsProgrammeUnemployedEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportSkillsProgrammeUnemployedEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportSkillsProgrammeUnemployedEntered(selectedQuarter);
					// commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataSkillsProgrammeUnemployedEnteredForQuarter(selectedQuarter);
						savedData = service.runSkillsProgrammeUnemployedEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runSkillsProgrammeUnemployedEnteredForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runSkillsProgrammeUnemployedEnteredForQuarterNotSaved(selectedQuarter);
