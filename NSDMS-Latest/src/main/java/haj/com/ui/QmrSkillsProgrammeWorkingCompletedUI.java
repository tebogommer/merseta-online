 ...
 */
@ManagedBean(name = "qmrSkillsProgrammeWorkingCompletedUI")
@ViewScoped
public class QmrSkillsProgrammeWorkingCompletedUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportSkillsProgrammeEmployedCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportSkillsProgrammeEmployedCompleted(selectedQuarter);
					// commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataSkillsProgrammeEmployedCompletedForQuarter(selectedQuarter);
						savedData = service.runSkillsProgrammeEmployedCompletedForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runSkillsProgrammeEmployedCompletedForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runSkillsProgrammeEmployedCompletedForQuarterNotSaved(selectedQuarter);
