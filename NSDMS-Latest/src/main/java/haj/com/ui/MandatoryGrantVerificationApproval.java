 ...
 */
@ManagedBean(name = "mandatoryGrantVerificationapprovalUI")
@ViewScoped
public class MandatoryGrantVerificationApproval extends AbstractUI {
	...
	private void runInit() throws Exception {
		mgVerification = mgVerificationService.findByKey(1L);
	...
	private void mgVerificationInfo() throws Exception {
		mgVerificationDataModel = new LazyDataModel<MgVerification>() {
			...
			public List<MgVerification> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
				try {
					retorno = mgVerificationService.sortAndFilterSearch(MgVerification.class, first, pageSize, sortField, sortOrder, filters);
					mgVerificationDataModel.setRowCount(mgVerificationService.countSearch(MgVerification.class, filters));
				} catch (Exception e) {
					logger.fatal(e);
					e.printStackTrace();
				}
	...
	{
		try {
			mgVerificationService.submitMgVerification(getSessionUI().getActiveUser(), mgVerification, getSessionUI().getTask());
			mgVerification = null;
		} catch (Exception e) {
			addErrorMessage(e.getMessage(),e);
		}
