 ...
 */
@ManagedBean(name = "qmrLearnershipWorkingEnteredUI")
@ViewScoped
public class QmrLearnershipWorkingEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportLearnershipEmployedEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportLearnershipEmployedEntered(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataLearnershipEmployedEnteredForQuarter(selectedQuarter);
						savedData = service.runLearnershipEmployedEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runLearnershipEmployedEnteredForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runLearnershipEmployedEnteredForQuarterNotSaved(selectedQuarter);
