 ...
 */
@ManagedBean(name = "qmrArtisanEnteredUI")
@ViewScoped
public class QmrArtisanEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportArtisanEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportArtisanEntered(selectedQuarter);
					// commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						///savedData = service.fetchSavedDataArtisanEnteredForQuarter(selectedQuarter);
						savedData = service.runArtisanEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runArtisanEnteredForQuarterNotSaved(selectedQuarter);
					} 
					//rawData = service.runArtisanEnteredForQuarterNotSaved(selectedQuarter);
