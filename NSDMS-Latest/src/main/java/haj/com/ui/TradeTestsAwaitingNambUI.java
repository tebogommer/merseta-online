...
@ManagedBean(name = "tradeTestsAwaitingNambUI")
@ViewScoped
public class TradeTestsAwaitingNambUI extends AbstractUI {
	...
	public void companyLearnersTradeTestDataModelInfo() {
		companyLearnersTradeTestDataModel = new LazyDataModel<CompanyLearnersTradeTest>() {
			...
			public List<CompanyLearnersTradeTest> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
				try {
					retorno = companyLearnersTradeTestService.allCompanyLearnersTradeTestByStatusAndType(first, pageSize, sortField, sortOrder, filters, TradeTestTypeEnum.TRADE_TEST, ApprovalEnum.AwaitingNAMB);
...
//			rejectReasonForm" oncomplete="PF('rejectReasonsDlg').show()"
			rejectionNumber = 1;
			selectedRejectionReasons = new ArrayList<>();
			rejectionReasonsSelection = reasonsService.findByProcess(ConfigDocProcessEnum.TradeTestApplication);
	...
	public void rejectArplTradeTestApplication(){
		try {
			if (selectedRejectionReasons == null || selectedRejectionReasons.size() == 0) {
				...
				case 1:
					// Reject to Admin to re upload. Work flow: ARPLTradeTestDocumentUpload
					companyLearnersTradeTestService.sendToAdminToReUpload(companyLearnersTradeTest, getSessionUI().getActiveUser(), selectedRejectionReasons);
					companyLearnersTradeTest = null;
					companyLearnersTradeTestDataModelInfo();
					addInfoMessage("Application Rejected Back Administrator");
					break;
				case 2:
					// Sends Trade Test back to test center. Work flow: TradeTestApplication
					...
					companyLearnersTradeTestService.sendTradeTestToTestCenter(companyLearnersTradeTest, getSessionUI().getActiveUser(), selectedRejectionReasons, signOffLists);
					companyLearnersTradeTest = null;
					companyLearnersTradeTestDataModelInfo();
	...
	public void submitNambApproval(){
		try {
			...
			if (errors.toString().isEmpty()) {
				companyLearnersTradeTestService.completeNambApprovalTradeTest(companyLearnersTradeTest, getSessionUI().getActiveUser(), supportingDocs);
