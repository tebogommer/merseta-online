 ...
 */
@ManagedBean(name = "qmrBursaryUnemployedEnteredUI")
@ViewScoped
public class QmrBursaryUnemployedEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportBursaryUnemployedEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportBursaryUnemployedEntered(selectedQuarter);
					/*commneted by vh
					 * if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						savedData = service.fetchSavedDataBursaryUnemployedEnteredForQuarter(selectedQuarter);
						
					} else {
						rawData = service.runBursaryUnemployedEnteredForQuarterNotSaved(selectedQuarter);
					}*/
