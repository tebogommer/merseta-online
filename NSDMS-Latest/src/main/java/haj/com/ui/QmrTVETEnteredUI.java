 ...
 */
@ManagedBean(name = "qmrTVETEnteredUI")
@ViewScoped
public class QmrTVETEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportTVETEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportTVETEntered(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataTVETEnteredForQuarter(selectedQuarter);
						  savedData = service.runTVETEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runTVETEnteredForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runTVETEnteredForQuarterNotSaved(selectedQuarter);
