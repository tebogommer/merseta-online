...
/**
 * The Class QmrLecturerDevelopmentEnteredUI.
 */
@ManagedBean(name = "qmrLecturerDevelopmentEnteredUI")
@ViewScoped
public class QmrLecturerDevelopmentEnteredUI extends AbstractUI {
	...
	private LazyDataModel<QmrLecturerDevelopmentData> savedDataModel;

	/* Array Lists */
	private List<Integer> distinctFinYearList = null;
	private List<QmrLecturerDevelopmentData> savedData = null;
	...
	public void generateSummaryReport() {
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportLecturerDevelopmentEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportLecturerDevelopmentEntered(selectedQuarter);
					/*commented by  vh
					 * if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						savedData = service.fetchSavedDataLecturerDevelopmentEnteredForQuarter(selectedQuarter);
					} else {
						rawData = service.runLecturerDevelopmentEnteredForQuarterNotSaved(selectedQuarter);
					}*/
					rawData = service.runLecturerDevelopmentEnteredForQuarterNotSaved(selectedQuarter);
	...
	public LazyDataModel<QmrLecturerDevelopmentData> getSavedDataModel() {
		return savedDataModel;
	}

	public void setSavedDataModel(LazyDataModel<QmrLecturerDevelopmentData> savedDataModel) {
	...
	public List<QmrLecturerDevelopmentData> getSavedData() {
		return savedData;
	}

	public void setSavedData(List<QmrLecturerDevelopmentData> savedData) {
