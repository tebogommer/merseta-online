...
/**
 * The Class QmrIndividualUnitStandardEnteredUI.
 */
@ManagedBean(name = "qmrIndividualUnitStandardEnteredUI")
@ViewScoped
public class QmrIndividualUnitStandardEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportIndividualUnitStandardEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportIndividualUnitStandardEntered(selectedQuarter);
					if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						savedData = service.fetchSavedDataIndividualUnitStandardEnteredForQuarter(selectedQuarter);
					} else {
						rawData = service.runIndividualUnitStandardEnteredForQuarterNotSaved(selectedQuarter);
					}
