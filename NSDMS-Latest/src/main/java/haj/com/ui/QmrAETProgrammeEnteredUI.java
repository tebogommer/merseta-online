...
/**
 * The Class QmrAETProgrammeEnteredUI.
 */
@ManagedBean(name = "qmrAETProgrammeEnteredUI")
@ViewScoped
public class QmrAETProgrammeEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport() {
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportAETProgrammeEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportAETProgrammeEntered(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataAETProgrammeEnteredForQuarter(selectedQuarter);
						savedData = service.runAETProgrammeEnteredForQuarterNotSaved(selectedQuarter);
						//savedData = service.runAETProgrammeEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runAETProgrammeEnteredForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runAETProgrammeEnteredForQuarterNotSaved(selectedQuarter);
