...
@ManagedBean(name = "arplNambRejectionUI")
@ViewScoped
public class ArplNambRejectionUI extends AbstractUI {
	...
	private List<RejectReasons> selectedRejectionReasonsList = new ArrayList<>();
	private List<RejectReasons> rejectReasonsSelectionList = new ArrayList<>();
	
	/* Vars */
	private Integer rejectIndicator = 0;
	...
	private void runInit() throws Exception {
		if (getSessionUI().getTask() != null && getSessionUI().getTask().getWorkflowProcess() == ConfigDocProcessEnum.ARPLTradeTestNambRejection) {
			...
			if (!TasksService.instance().taskOpenBasedOnStatus(getSessionUI().getTask().getTaskStatus())) {
				// redirect if task not open
				ajaxRedirectToDashboard();
			} else {
				// Task open and correct work flow
				...
				companyLearnersTradeTest = companyLearnersTradeTestService.populateDocsByTargetClassAndKey(companyLearnersTradeTestService.findByKey(companyLearnersTradeTest.getId()));
	...
	private void populateRejectionReasons() throws Exception{
		rejectionReasons = rejectReasonsService.locateReasonsSelectedByTargetKeyClassAndProcess(companyLearnersTradeTest.getId(), companyLearnersTradeTest.getClass().getName(), ConfigDocProcessEnum.ARPLTradeTestNambRejection);
	}
	...
	public void downloadLPMFM009() {
		try {
			...
			companyLearnersTradeTestService.downloadLPMFM009(companyLearnersTradeTest, getSessionUI().getActiveUser());
		} catch (Exception e) {
			addErrorMessage(e.getMessage(), e);
			e.printStackTrace();
		}
	...
	public void submitToQaManager(){
		try {
			companyLearnersTradeTestService.completeTaskArplRejection(companyLearnersTradeTest, getSessionUI().getActiveUser(), getSessionUI().getTask());
			ajaxRedirectToDashboard();
		} catch (Exception e) {
			addErrorMessage(e.getMessage(), e);
		}
	...
	public void prepRejectToQa(){
		try {
			rejectIndicator = 1;
			selectedRejectionReasonsList = new ArrayList<>();
			rejectReasonsSelectionList = rejectReasonsService.findByProcess(ConfigDocProcessEnum.ARPLTradeTestNambRejection);
		} catch (Exception e) {
			addErrorMessage(e.getMessage(), e);
		}
	}

	public void rejectToQA(){
		try {
			if (selectedRejectionReasonsList == null || selectedRejectionReasonsList.size() == 0) {
				addErrorMessage("Provide Atleast One Rejection Reason Before Proceeding.");
			} else {
				companyLearnersTradeTestService.rejectArplRejectionToRegionQualityAssurance(companyLearnersTradeTest, getSessionUI().getActiveUser(), getSessionUI().getTask(), selectedRejectionReasonsList);
				ajaxRedirectToDashboard();
			}
		} catch (Exception e) {
			addErrorMessage(e.getMessage(), e);
	...
	public void finalRejectTradeTestApplication(){
		try {
			if (selectedRejectionReasonsList == null || selectedRejectionReasonsList.size() == 0) {
				addErrorMessage("Provide Atleast One Rejection Reason Before Proceeding.");
			} else {
				companyLearnersTradeTestService.finalRejectApplicationQaManager(companyLearnersTradeTest, getSessionUI().getActiveUser(), getSessionUI().getTask(), selectedRejectionReasonsList);
				ajaxRedirectToDashboard();
			}
		} catch (Exception e) {
			addErrorMessage(e.getMessage(), e);
	...
	public void approveArplNambRejection(){
		try {
			companyLearnersTradeTestService.approveArplNambRejection(companyLearnersTradeTest, getSessionUI().getActiveUser(), getSessionUI().getTask());
			ajaxRedirectToDashboard();
		} catch (Exception e) {
			addErrorMessage(e.getMessage(), e);
		}
	...
	public List<RejectReasons> getSelectedRejectionReasonsList() {
		return selectedRejectionReasonsList;
	}

	public void setSelectedRejectionReasonsList(List<RejectReasons> selectedRejectionReasonsList) {
		this.selectedRejectionReasonsList = selectedRejectionReasonsList;
	}

	public Integer getRejectIndicator() {
		return rejectIndicator;
	}

	public void setRejectIndicator(Integer rejectIndicator) {
		this.rejectIndicator = rejectIndicator;
	}

	public List<RejectReasons> getRejectReasonsSelectionList() {
		return rejectReasonsSelectionList;
	}

	public void setRejectReasonsSelectionList(List<RejectReasons> rejectReasonsSelectionList) {
		this.rejectReasonsSelectionList = rejectReasonsSelectionList;
	}
