...
/**
 * The Class PrimarySdpDelinkUI.
 */
@ManagedBean(name = "primarySdpDelinkUI")
@ViewScoped
public class PrimarySdpDelinkUI extends AbstractUI {
	...
	public void searchTrainingProviderApplication(){
		try {
			if (accredNumberEntered.trim().isEmpty()) {
				...
				// check if primary SDP assigned, if not error
				if (trainingSite != null && trainingSite.getId() != null) {
					if (sdpCompanyService.countSdpTypeByTrainingSiteIdAndApproval(trainingProviderApplication.getTrainingSite().getId(), HAJConstants.PRIMARY_SDP_ID, ApprovalEnum.Approved) == 0) {
						failedProviderSearch();
						throw new Exception("No Approved Primary SDP Contact Person Assigned.");
					}
					primarySdpLinked = sdpCompanyService.findBySdpTypeByTrainingSiteId(trainingProviderApplication.getTrainingSite().getId(), HAJConstants.PRIMARY_SDP_ID);
				} else {
					if (sdpCompanyService.countSdpTypeByHoldingCompanyAndApprovalStatus(company.getId(), HAJConstants.PRIMARY_SDP_ID, ApprovalEnum.Approved) == 0) {
						failedProviderSearch();
						throw new Exception("No Approved Primary SDP Contact Person Assigned.");
					}
					primarySdpLinked = sdpCompanyService.findBySdpTypeByHoldingCompany(company.getId(), HAJConstants.PRIMARY_SDP_ID);
				}
	...
	public void submitSdpRemoval(){
		try {
			...
			sdpCompanyActions.setSdpCompanyAction(SdpCompanyActionListEnum.RemoveSDP);
			sdpCompanyActions.setTrainingProviderApplication(trainingProviderApplication);
			sdpCompanyActions.setCompany(company);
			sdpCompanyActions.setSdpCompanyFlatId(primarySdpLinked.getId());
			sdpCompanyActions.setCurrentDesignation(primarySdpLinked.getSdpType());
			sdpCompanyActions.setSdpUser(primarySdpLinked.getSdp());
			if (trainingSite != null && trainingSite.getId() != null) {
				sdpCompanyActions.setTrainingSite(trainingSite);
			}
			sdpCompanyActionsService.initiateWorkflow(sdpCompanyActions, primarySdpLinked.getSdp(), primarySdpLinked, trainingProviderApplication, formUser, docList);
			failedProviderSearch();
			clearUserSearch();
			addInfoMessage("Action Complete");
		} catch (Exception e) {
