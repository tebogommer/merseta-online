 ...
 */
@ManagedBean(name = "qmrUniversityStudentEnteredUI")
@ViewScoped
public class QmrUniversityStudentEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportUniversityStudentEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportUniversityStudentEntered(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataUniversityStudentEnteredForQuarter(selectedQuarter);
						savedData = service.runUniversityStudentEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runUniversityStudentEnteredForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runUniversityStudentEnteredForQuarterNotSaved(selectedQuarter);
