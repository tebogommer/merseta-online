...
/**
 * The Class QmrIndividualUnitStandardCompletedUI.
 */
@ManagedBean(name = "qmrIndividualUnitStandardCompletedUI")
@ViewScoped
public class QmrIndividualUnitStandardCompletedUI extends AbstractUI {
	...
	private LazyDataModel<QmrIndividualUnitStandardData> savedDataModel;
	
	/* Array Lists */
	private List<Integer> distinctFinYearList = null;
	private List<QmrIndividualUnitStandardData> savedData = null;
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportIndividualUnitStandardCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportIndividualUnitStandardCompleted(selectedQuarter);
					if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						savedData = service.fetchSavedDataIndividualUnitStandardCompletedForQuarter(selectedQuarter);
					} else {
						rawData = service.runIndividualUnitStandardCompletedForQuarterNotSaved(selectedQuarter);
					}
	...
	public LazyDataModel<QmrIndividualUnitStandardData> getSavedDataModel() {
		return savedDataModel;
	}

	public void setSavedDataModel(LazyDataModel<QmrIndividualUnitStandardData> savedDataModel) {
	...
	public List<QmrIndividualUnitStandardData> getSavedData() {
		return savedData;
	}

	public void setSavedData(List<QmrIndividualUnitStandardData> savedData) {
