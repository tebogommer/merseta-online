 ...
 */
@ManagedBean(name = "qmrLearnershipWorkingCompletedUI")
@ViewScoped
public class QmrLearnershipWorkingCompletedUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportLearnershipEmployedCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportLearnershipEmployedCompleted(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataLearnershipEmployedCompletedForQuarter(selectedQuarter);
						savedData = service.runLearnershipEmployedCompletedForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runLearnershipEmployedCompletedForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runLearnershipEmployedCompletedForQuarterNotSaved(selectedQuarter);
