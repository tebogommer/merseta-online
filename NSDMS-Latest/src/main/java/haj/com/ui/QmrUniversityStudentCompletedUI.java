...
/**
 * The Class QmrUniversityStudentEnteredUI.
 */
@ManagedBean(name = "qmrUniversityStudentCompletedUI")
@ViewScoped
public class QmrUniversityStudentCompletedUI extends AbstractUI {
	...
	private LazyDataModel<QmrUniversityStudentData> savedDataModel;
	
	/* Array Lists */
	private List<Integer> distinctFinYearList = null;
	//private List<QmrUniversityStudentData> savedData = null;
	private List<QmrScriptFourBean> savedData = null;
	private List<QmrScriptFourBean> rawData = null;
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportUniversityStudentCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportUniversityStudentCompleted(selectedQuarter);
					//commented by vh
					 if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataUniversityStudentCompletedForQuarter(selectedQuarter);
						savedData = service.runUniversityStudentCompletedForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runUniversityStudentCompletedForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runUniversityStudentCompletedForQuarterNotSaved(selectedQuarter);
	...
	public LazyDataModel<QmrUniversityStudentData> getSavedDataModel() {
		return savedDataModel;
	}

	public void setSavedDataModel(LazyDataModel<QmrUniversityStudentData> savedDataModel) {
		this.savedDataModel = savedDataModel;
	}

	public List<QmrScriptFourBean> getRawData() {
		return rawData;
	}

	public void setRawData(List<QmrScriptFourBean> rawData) {
	...
	public List<QmrScriptFourBean> getSavedData() {
		return savedData;
	}

	public void setSavedData(List<QmrScriptFourBean> savedData) {
