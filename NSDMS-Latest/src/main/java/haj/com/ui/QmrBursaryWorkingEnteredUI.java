 ...
 */
@ManagedBean(name = "qmrBursaryWorkingEnteredUI")
@ViewScoped
public class QmrBursaryWorkingEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportBursaryEmployedEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportBursaryEmployedEntered(selectedQuarter);
					// commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataBursaryEmployedEnteredForQuarter(selectedQuarter);
						savedData = service.runBursaryEmployedEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runBursaryEmployedEnteredForQuarterNotSaved(selectedQuarter);
					}  
					//rawData = service.runBursaryEmployedEnteredForQuarterNotSaved(selectedQuarter);
