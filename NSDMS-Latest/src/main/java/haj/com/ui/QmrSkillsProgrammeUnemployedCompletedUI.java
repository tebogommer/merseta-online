...
/**
 * The Class QmrSkillsProgrammeUnemployedEnteredUI.
 */
@ManagedBean(name = "qmrSkillsProgrammeUnemployedCompletedUI")
@ViewScoped
public class QmrSkillsProgrammeUnemployedCompletedUI extends AbstractUI {
	...
	private LazyDataModel<QmrSkillsProgrammeData> savedDataModel;
	
	/* Array Lists */
	private List<Integer> distinctFinYearList = null;
	//private List<QmrSkillsProgrammeData> savedData = null;
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportSkillsProgrammeUnemployedCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportSkillsProgrammeUnemployedCompleted(selectedQuarter);
					// commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataSkillsProgrammeUnemployedCompletedForQuarter(selectedQuarter);
						savedData = service.runSkillsProgrammeUnemployedCompletedForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runSkillsProgrammeUnemployedCompletedForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runSkillsProgrammeUnemployedCompletedForQuarterNotSaved(selectedQuarter);
	...
	public LazyDataModel<QmrSkillsProgrammeData> getSavedDataModel() {
		return savedDataModel;
	}

	public void setSavedDataModel(LazyDataModel<QmrSkillsProgrammeData> savedDataModel) {
