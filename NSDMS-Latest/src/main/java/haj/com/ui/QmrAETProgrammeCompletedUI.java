...
/**
 * The Class QmrAETProgrammeCompletedUI.
 */
@ManagedBean(name = "qmrAETProgrammeCompletedUI")
@ViewScoped
public class QmrAETProgrammeCompletedUI extends AbstractUI {
	...
	private LazyDataModel<QmrAETProgrammeData> savedDataModel;
	
	/* Array Lists */
	private List<Integer> distinctFinYearList = null;
	//private List<QmrAETProgrammeData> savedData = null;
	private List<QmrScriptFiveBean> savedData = null;
	private List<QmrScriptFiveBean> rawData = null;
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportAETProgrammeCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportAETProgrammeCompleted(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataAETProgrammeCompletedForQuarter(selectedQuarter);
						savedData = service.runAETProgrammeCompletedForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runAETProgrammeCompletedForQuarterNotSaved(selectedQuarter);
					} 
					//rawData = service.runAETProgrammeCompletedForQuarterNotSaved(selectedQuarter);
	...
	public LazyDataModel<QmrAETProgrammeData> getSavedDataModel() {
		return savedDataModel;
	}

	public void setSavedDataModel(LazyDataModel<QmrAETProgrammeData> savedDataModel) {
		this.savedDataModel = savedDataModel;
	}

	public List<QmrScriptFiveBean> getRawData() {
		return rawData;
	}

	public void setRawData(List<QmrScriptFiveBean> rawData) {
	...
	public List<QmrScriptFiveBean> getSavedData() {
		return savedData;
	}

	public void setSavedData(List<QmrScriptFiveBean> savedData) {
