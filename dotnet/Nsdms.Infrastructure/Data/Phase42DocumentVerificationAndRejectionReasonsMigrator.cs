...
/// <summary>
/// Idempotent database schema migrator provisioning document verification attributes on DocumentAttachment
/// and creating the DocumentRejectionReasonType lookup catalog with pre-seeded standard reasons.
/// </summary>
public static class Phase42DocumentVerificationAndRejectionReasonsMigrator
{
    public static async Task MigrateAsync(IServiceProvider serviceProvider)
    {
        ...
        {
            // Try loading external SQL script file first, with inline fallback
            var scriptPaths = new[]
            {
                Path.Combine(AppContext.BaseDirectory, "Data", "SqlScripts", "V2026_42_Document_Verification_And_Rejection_Reasons.sql"),
                Path.Combine(AppContext.BaseDirectory, "SqlScripts", "V2026_42_Document_Verification_And_Rejection_Reasons.sql"),
                Path.Combine(Directory.GetCurrentDirectory(), "..", "Nsdms.Infrastructure", "Data", "SqlScripts", "V2026_42_Document_Verification_And_Rejection_Reasons.sql"),
                Path.Combine(Directory.GetCurrentDirectory(), "Data", "SqlScripts", "V2026_42_Document_Verification_And_Rejection_Reasons.sql")
            };
            ...
            try
            {
                ...
                logger?.LogInformation("Phase 42 Document Verification & Rejection Reasons catalog migrated successfully.");
            }
            catch (Exception ex)
            {
                logger?.LogError(ex, "Phase 42 migration encountered an error executing SQL script.");
                throw;
            }
        }
    }
