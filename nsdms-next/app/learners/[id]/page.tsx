  ...
  return (
    <main className="min-h-screen bg-muted/50 p-6 md:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="sticky top-0 z-10 flex flex-col md:flex-row md:items-center justify-between bg-card text-card-foreground px-6 py-4 rounded-xl border border-border shadow-sm">
          <div className="flex items-center gap-4 mb-4 md:mb-0">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-xl font-bold text-foreground">{learner.user?.name || "Unknown Learner"}</h1>
        ...
        <Tabs defaultValue="general" className="w-full">
          <TabsList className="bg-card text-card-foreground border text-muted-foreground h-12 p-1 rounded-lg w-full md:w-auto shadow-sm grid grid-cols-3 md:flex md:mb-6 mb-4">
          ...
          <TabsContent value="general" className="mt-0 outline-none">
            <div className="bg-card text-card-foreground rounded-xl shadow-sm border border-border p-6">
              <h3 className="text-lg font-bold text-foreground flex items-center gap-2 border-b pb-3 mb-6">
                <FileText className="w-5 h-5 text-muted-foreground" /> Learner Identity & Profile
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  <div>
                    ...
                    <p className="font-medium text-foreground mt-1">{learner.user?.name?.split(' ')[0] || "N/A"}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Last Name</label>
                    <p className="font-medium text-foreground mt-1">{learner.user?.name?.split(' ').slice(1).join(' ') || "N/A"}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Email</label>
                    <p className="font-medium text-foreground mt-1">{learner.user?.email || "N/A"}</p>
                  </div>
                </div>
                <div className="space-y-6">
                  <div>
                    <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">RSA ID Number</label>
                    <p className="font-medium text-foreground mt-1">{learner.rsaIdNumber || "N/A"}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Passport Number</label>
                    <p className="font-medium text-foreground mt-1">{learner.passportNumber || "N/A"}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Nationality</label>
                    <p className="font-medium text-foreground mt-1">{learner.nationality || "N/A"}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-card text-card-foreground rounded-xl shadow-sm border border-border overflow-hidden">
                <div className="p-6 border-b border-border flex items-center justify-between">
                    <h3 className="text-lg font-bold text-foreground flex items-center gap-2">
                        <Network className="w-5 h-5 text-muted-foreground" /> Active Enrollments
                    </h3>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm">
                        <thead className="bg-muted text-xs uppercase font-semibold text-muted-foreground tracking-wider">
                            <tr>
