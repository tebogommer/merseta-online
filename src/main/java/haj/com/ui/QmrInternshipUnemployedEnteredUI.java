 ...
 */
@ManagedBean(name = "qmrInternshipUnemployedEnteredUI")
@ViewScoped
public class QmrInternshipUnemployedEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportInternshipUnemployedEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportInternshipUnemployedEntered(selectedQuarter);
					// commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataInternshipUnemployedEnteredForQuarter(selectedQuarter);
						savedData = service.runInternshipUnemployedEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runInternshipUnemployedEnteredForQuarterNotSaved(selectedQuarter);
					}
