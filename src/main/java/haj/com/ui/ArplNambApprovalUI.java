...
@ManagedBean(name = "arplNambApprovalUI")
@ViewScoped
public class ArplNambApprovalUI extends AbstractUI {
	...
	private void runInit() throws Exception {
		if (getSessionUI().getTask() != null && getSessionUI().getTask().getWorkflowProcess() == ConfigDocProcessEnum.ARPLTradeTestNambApproval) {
		...
		try {
			if (getSessionUI().getTask().getFirstInProcess() != null && getSessionUI().getTask().getFirstInProcess()) {
				...
				if (errors.toString().isEmpty()) {
					companyLearnersTradeTestService.completeTask(companyLearnersTradeTest, getSessionUI().getActiveUser(), getSessionUI().getTask(), canUpload, canEdit);
					ajaxRedirectToDashboard();
				} else{
