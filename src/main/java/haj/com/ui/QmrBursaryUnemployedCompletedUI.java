 ...
 */
@ManagedBean(name = "qmrBursaryUnemployedCompletedUI")
@ViewScoped
public class QmrBursaryUnemployedCompletedUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportBursaryUnemployedCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportBursaryUnemployedCompleted(selectedQuarter);
					/*
					 commneted by vh
						savedData = service.fetchSavedDataBursaryUnemployedCompletedForQuarter(selectedQuarter);
					} else {
						rawData = service.runBursaryUnemployedCompletedForQuarterNotSaved(selectedQuarter);
					}*/
					System.out.println("bursary unemp completed");
					rawData = service.runBursaryUnemployedCompletedForQuarterNotSaved(selectedQuarter);
	...
	public List<QmrBursaryData> getSavedData() {
		return savedData;
	}

	public void setSavedData(List<QmrBursaryData> savedData) {
