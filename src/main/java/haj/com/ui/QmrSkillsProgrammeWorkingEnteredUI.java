 ...
 */
@ManagedBean(name = "qmrSkillsProgrammeWorkingEnteredUI")
@ViewScoped
public class QmrSkillsProgrammeWorkingEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportSkillsProgrammeEmployedEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportSkillsProgrammeEmployedEntered(selectedQuarter);
					// commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataSkillsProgrammeEmployedEnteredForQuarter(selectedQuarter);
						savedData = service.runSkillsProgrammeEmployedEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runSkillsProgrammeEmployedEnteredForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runSkillsProgrammeEmployedEnteredForQuarterNotSaved(selectedQuarter);
