...
/**
 * The Class QmrCandidacyProgrammeEnteredUI.
 */
@ManagedBean(name = "qmrCandidacyProgrammeCompletedUI")
@ViewScoped
public class QmrCandidacyProgrammeCompletedUI extends AbstractUI {
	...
	private LazyDataModel<QmrCandidacyProgrammeData> savedDataModel;
	
	/* Array Lists */
	private List<Integer> distinctFinYearList = null;
	//private List<QmrCandidacyProgrammeData> savedData = null;
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = QmrGenericService.instance().getQmrSummaryReportTemplate();
	...
	public void clearSummaryReport(){
		try {
			qmrSummaryBeanList = service.generateSummaryReportCandidacyProgrammeCompleted(selectedFinYear);
			addInfoMessage("Summary Report Generated");
		} catch (Exception e) {
			addErrorMessage(e.getMessage(), e);
		}
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportCandidacyProgrammeCompleted(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataCandidacyProgrammeCompletedForQuarter(selectedQuarter);
						savedData = service.runCandidacyProgrammeCompletedForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runCandidacyProgrammeCompletedForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runCandidacyProgrammeCompletedForQuarterNotSaved(selectedQuarter);
	...
	public LazyDataModel<QmrCandidacyProgrammeData> getSavedDataModel() {
		return savedDataModel;
	}

	public void setSavedDataModel(LazyDataModel<QmrCandidacyProgrammeData> savedDataModel) {
