 ...
 */
@ManagedBean(name = "employeeTemplateViewUI")
@ViewScoped
public class EmployeeTemplateViewUI extends AbstractUI {

	/**  Entity Layer. */
	private HostingCompany hostingCompany;
	
	/** The selected template. */
	private HostingCompanyEmployeeTemplates selectedTemplate;
	...
	private HostingCompanyEmployeeTemplatesService HostingCompanyEmployeeTemplatesService = new HostingCompanyEmployeeTemplatesService();
	...
	private void userCheck() throws Exception{
		...
		if (getSessionUI().getActiveUser().getSuperAdmin() != null && getSessionUI().getActiveUser().getSuperAdmin()) {
			canAccess = true;
		}
	...
	private TreeNode buildTree() throws Exception {
		...
		HostingCompanyEmployeeTemplatesService.startBuild(leftRoot, getSessionUI().getActiveUser(), hostingCompany, expand,
				HostingCompanyEmployeeTemplatesService.findRoots(hostingCompany));
		return leftRoot;
	}
...
//			showDocForm = false;
	...
	public void onNodeSelect(NodeSelectEvent event) {
		try {
			this.selectedTemplate = (HostingCompanyEmployeeTemplates) event.getTreeNode().getData();
	...
	private void findDocs() throws Exception {
		this.documentList = docService.searchByTargetKeyAndClass(selectedTemplate.getClass().getName(), selectedTemplate.getId());
	}
	 ...
	 */
	public HostingCompanyEmployeeTemplates getSelectedTemplate() {
	 ...
	 */
	public void setSelectedTemplate(HostingCompanyEmployeeTemplates selectedTemplate) {
