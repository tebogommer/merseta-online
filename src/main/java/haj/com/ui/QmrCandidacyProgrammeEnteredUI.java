 ...
 */
@ManagedBean(name = "qmrCandidacyProgrammeEnteredUI")
@ViewScoped
public class QmrCandidacyProgrammeEnteredUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportCandidacyProgrammeEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportCandidacyProgrammeEntered(selectedQuarter);
					// commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataCandidacyProgrammeEnteredForQuarter(selectedQuarter);
						savedData = service.runCandidacyProgrammeEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runCandidacyProgrammeEnteredForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runCandidacyProgrammeEnteredForQuarterNotSaved(selectedQuarter);
