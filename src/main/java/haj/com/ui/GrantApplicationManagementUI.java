...
@ManagedBean(name = "grantApplicationManagementUI")
@ViewScoped
public class GrantApplicationManagementUI extends AbstractUI {
	...
	public void selectCompany() {
		try {
			...
			// locate previous company information
			if (selectedCompany != null && selectedCompany.getPreviousCompany() != null && selectedCompany.getPreviousCompany().getId() != null) {
				previousCompany = companyService.findByKey(selectedCompany.getPreviousCompany().getId());
...
//				case SdfSignoff:
	...
	/*
	 * Only available for year: 2019
	 */
	public void prepareLinkedCompanyWsp() {
		try {
			// validate there is no 2019 grant application
			boolean passedValidiation = false;
			if (wspService.countByCompanyAndFinYear(selectedCompany, 2019) == 0) {
				if (previousCompany != null && previousCompany.getId() != null) {
					if (wspService.countByCompanyAndFinYear(previousCompany, 2019) == 0) {
						passedValidiation = true;
					} else {
						passedValidiation = false;
						throw new Exception("Unable to proceed. Grant Application For Previous Transacting Company Year: 2019 Has Been Found For " + previousCompany.getLevyNumber());
					}
				} else {
					passedValidiation = true;
				}
			} else {
				throw new Exception("Unable to proceed. Grant Application For Year: 2019 Has Been Found For "+selectedCompany.getLevyNumber());
			}
			
			if (passedValidiation) {
				this.newWsp = new Wsp();
				this.newWsp.setFinYear(2019);
				this.newWsp.setCompany(selectedCompany);
				this.newWsp.setCreateUsers(getSessionUI().getActiveUser());
				this.newWsp.setWspType(WspTypeEnum.LegacyLinkedGrantApplication);
				wspService.prepareDocForNewWspByWspDocList(ConfigDocProcessEnum.WSP, this.newWsp);
				runClientSideExecute("PF('dlgLinkedCompanyWsp').show()");
			}
	...
	public void uploadDocMemory(FileUploadEvent event) {
		doc.setData(event.getFile().getContents());
		doc.setOriginalFname(event.getFile().getFileName());
		doc.setExtension(FilenameUtils.getExtension(doc.getOriginalFname()));
		super.runClientSideExecute("PF('dlgUploadDoc').hide()");
	}
	
	public void closeLinkedCompanyWsp(){
		try {
			newWsp = null;
			runClientSideExecute("PF('dlgLinkedCompanyWsp').show()");
		} catch (Exception e) {
	...
	public void initiateWSP() {
		try {
			...
			selectCompany();
			addInfoMessage("Grant Created for Year: " + newWsp.getFinYear());
//			this.newWsp  = null;
		} catch (ValidationException e) {
			addErrorMessage(getEntryLanguage(e.getMessage(), e.getParams()));
		} catch (Exception e) {
	...
	public void initiateLinkedCompanyWsp() {
		try {
			wspService.checkWspFinYearUnique(this.newWsp);
			wspService.createLinkedWsp(this.newWsp, getSessionUI().getUser());
			// send notification 
			runClientSideExecute("PF('dlgLinkedCompanyWsp').show()");
			addInfoMessage("Grant Created for Year: " + newWsp.getFinYear());
			this.newWsp  = null;
