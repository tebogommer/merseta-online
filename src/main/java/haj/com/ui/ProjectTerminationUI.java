...
@ManagedBean(name = "projectTerminationUI")
@ViewScoped
public class ProjectTerminationUI extends AbstractUI {
	...
	private void runInit() throws Exception {
		if (getSessionUI().getTask() != null && getSessionUI().getTask().getWorkflowProcess() == ConfigDocProcessEnum.DG_PROJECT_TERMINATION) {
	...
	public void completeWorkflow() {
		try {
			service.completeWorkflowProjectTermination(activecontracts, getSessionUI().getActiveUser(), getSessionUI().getTask());
	...
	public void rejectWorkflow() {
		try {
			...
			service.rejectWorkflowProjectTermination(activecontracts, getSessionUI().getActiveUser(), getSessionUI().getTask(), selectedRejectReason);
			ajaxRedirectToDashboard();
		} catch (Exception e) {
			e.printStackTrace();
			addErrorMessage(e.getMessage(), e);
	...
	public List<RejectReasons> getRejectReasonsSeniorManager() {
		...
		List<RejectReasons> l = null;
		try {
			l = rejectReasonsService.findByProcessSeniorManager(ConfigDocProcessEnum.DG_PROJECT_TERMINATION, booleanValue);
		} catch (Exception e) {
			addErrorMessage(e.getMessage(), e);
		}
		return l;
