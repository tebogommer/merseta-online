...
/**
 * The Class QmrRPLEnteredUI.
 */
@ManagedBean(name = "qmrRPLUI")
@ViewScoped
public class QmrRPLUI extends AbstractUI {
	...
	private LazyDataModel<QmrRPLData> savedDataModel;
	
	/* Array Lists */
	private List<Integer> distinctFinYearList = null;
	//private List<QmrRPLData> savedData = null;
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportRPLEntered(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportRPLEntered(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataRPLEnteredForQuarter(selectedQuarter);
						  savedData = service.runRPLEnteredForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runRPLEnteredForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runRPLEnteredForQuarterNotSaved(selectedQuarter);
	...
	public LazyDataModel<QmrRPLData> getSavedDataModel() {
		return savedDataModel;
	}

	public void setSavedDataModel(LazyDataModel<QmrRPLData> savedDataModel) {
