...
/**
 * The Class QmrTVETEnteredUI.
 */
@ManagedBean(name = "qmrTVETCompletedUI")
@ViewScoped
public class QmrTVETCompletedUI extends AbstractUI {
	...
	private LazyDataModel<QmrTVETData> savedDataModel;
	
	/* Array Lists */
	private List<Integer> distinctFinYearList = null;
	//private List<QmrTVETData> savedData = null;
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportTVETCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportTVETCompleted(selectedQuarter);
					//*commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataTVETCompletedForQuarter(selectedQuarter);
						savedData = service.runTVETCompletedForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runTVETCompletedForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runTVETCompletedForQuarterNotSaved(selectedQuarter);
	...
	public LazyDataModel<QmrTVETData> getSavedDataModel() {
		return savedDataModel;
	}

	public void setSavedDataModel(LazyDataModel<QmrTVETData> savedDataModel) {
