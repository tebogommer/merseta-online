 ...
 */
@ManagedBean(name = "qmrLearnershipUnemployedCompletedUI")
@ViewScoped
public class QmrLearnershipUnemployedCompletedUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportLearnershipUnemployedCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportLearnershipUnemployedCompleted(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataLearnershipUnemployedCompletedForQuarter(selectedQuarter);
						savedData = service.runLearnershipUnemployedCompletedForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runLearnershipUnemployedCompletedForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runLearnershipUnemployedCompletedForQuarterNotSaved(selectedQuarter);
