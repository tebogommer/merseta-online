...
@ManagedBean(name = "monitorTrainingProvidersUI")
@ViewScoped
public class MonitorTrainingProvidersUI extends AbstractUI {
	...
	public void dataModelTrainingProviderApplicationInfo() {
		dataModelTrainingProviderApplication = new LazyDataModel<TrainingProviderApplication>() {
			...
			public List<TrainingProviderApplication> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
				try {
					if (Boolean.TRUE.equals(filterByApprovedApplications)) {
						retorno = service.sortAndFilterWhereTPInfoWhereApproved(first, pageSize, sortField, sortOrder, filters);
						dataModelTrainingProviderApplication.setRowCount(service.countTPInfoWhereApproved(filters));
					} else {
						retorno = service.sortAndFilterWhereTPInfo(first, pageSize, sortField, sortOrder, filters);
						dataModelTrainingProviderApplication.setRowCount(service.countWhereTPInfo(filters));
					}
				} catch (Exception e) {
					logger.fatal(e);
				}
