...
@ManagedBean(name = "viewCreateArplUI")
@ViewScoped
public class ViewCreateArplUI extends AbstractUI {
	...
	public void companyLearnersTradeTestDataModelInfo() {
		companyLearnersTradeTestDataModel = new LazyDataModel<CompanyLearnersTradeTest>() {
			...
			public List<CompanyLearnersTradeTest> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
				try {
					retorno = companyLearnersTradeTestService.allCompanyLearnersTradeTestByLearnerId(first, pageSize, sortField, sortOrder, filters, getSessionUI().getActiveUser(), TradeTestTypeEnum.ARPL);
					companyLearnersTradeTestDataModel.setRowCount(companyLearnersTradeTestService.countAllCompanyLearnersTradeTestByLearnerId(filters));
				} catch (Exception e) {
					logger.fatal(e);
				}
				return retorno;
...
//			companyLearnersTradeTest.setCreateUser(usersService.findByKey(1460));			
	...
	public void requestARPLTradeTestApplicationLearnerSide() {
		try {
			validateInformation(companyLearnersTradeTest);
//			companyLearnersTradeTest.setCreateUser(usersService.findByKey(1460));			
			companyLearnersTradeTest.setCreateUser(getSessionUI().getActiveUser());
