...
@ManagedBean(name = "learnerRegistrationNonMersetaFormUI")
@ViewScoped
public class LearnerRegistrationNonMersetaFormUI extends AbstractUI {
	...
	private void runInit() throws Exception {
		...
		if (super.getParameter("id", false) != null) {
			if (getSessionUI().getTask() != null && getSessionUI().getTask().getWorkflowProcess() == ConfigDocProcessEnum.LearnerRegistrationByNonMersetaCompanies) {
				getSessionUI().setTask(TasksService.instance().findByKey(getSessionUI().getTask().getId()));
				this.companylearners = companyLearnersService.findByKey(getSessionUI().getTask().getTargetKey(), ConfigDocProcessEnum.LearnerRegistrationByNonMersetaCompanies);
				companyLearnersService.prepareNewRegistration(ConfigDocProcessEnum.LearnerRegistrationByNonMersetaCompanies, companylearners);
				...
				if(getSessionUI().getTask() != null && getSessionUI().getTask().getProcessRole() !=null && getSessionUI().getTask().getFirstInProcess() ) {
					showTP = false;
				}else {
					showTP = true;
				}
	...
	private void populateRejectReasons() {
		...
		try {
			rejectReason = rs.locateReasonsSelectedByTargetKeyClassAndProcess(companylearners.getId(), CompanyLearners.class.getName(), ConfigDocProcessEnum.LearnerRegistrationByNonMersetaCompanies);			
		} catch (Exception e) {
			e.printStackTrace();
			addErrorMessage(e.getMessage(), e);
		}		
	...
	public void finalApproveWorkflow() {
		try {
			...
			nonSetaCompanyService.finalApproveWorkflow(companylearners, getSessionUI().getActiveUser(), getSessionUI().getTask(), documentBoxID);
			ajaxRedirectToDashboard();
	...
	public void createLearnerWorkflow() {
		try {
			nonSetaCompanyService.createLearnerWorkflow(companylearners, getSessionUI().getActiveUser(), getSessionUI().getTask());
			ajaxRedirectToDashboard();
		}  catch (Exception e) {
			e.printStackTrace();
			addErrorMessage(e.getMessage(), e);
	...
	public List<RejectReasons> getRejectReasonsByProcess() {
		...
		try {
			l = rejectReasonsService.findByProcess(ConfigDocProcessEnum.LearnerRegistrationByNonMersetaCompanies);
		} catch (Exception e) {
			e.printStackTrace();
			addErrorMessage(e.getMessage(), e);
		}
	...
	private void startLearnerReg(boolean sendWorkflow) throws Exception {
		...
		nonSetaCompanyService.createLearner(getSessionUI().getActiveUser(), this.user, company, trainingProvider, companylearners, sendWorkflow, requireGaurdian, usersLanguageList, usersDisabilityList);
	...
	public void rejectCompanyLearner() {
		try {
			...
			nonSetaCompanyService.rejectCompanyLearner(companylearners, getSessionUI().getActiveUser(), getSessionUI().getTask(), selectedRejectReason);
			ajaxRedirectToDashboard();
		} catch (Exception e) {
			e.printStackTrace();
			addErrorMessage(e.getMessage(), e);
	...
	public void finalApproveCompanyLearners() {
		try {
			if (signoff) {
				...
				//(CompanyLearners companyLearners, Company entity, Users user, Tasks tasks, String documentBoxID) 
				nonSetaCompanyService.finalApproveWorkflow(companylearners, getSessionUI().getActiveUser(), getSessionUI().getTask(), documentBoxID);
				ajaxRedirectToDashboard();
			} else {
	...
	public void finalRejectCompanyLearners() {
		try {
			if (selectedRejectReason.size() == 0) throw new Exception("Please select a reject reason");
			nonSetaCompanyService.finalRejectCompanyLearners(companylearners, getSessionUI().getActiveUser(), getSessionUI().getTask(), selectedRejectReason);
			ajaxRedirectToDashboard();
		} catch (Exception e) {
			e.printStackTrace();
			addErrorMessage(e.getMessage(), e);
	...
	public void prepareUserAfterSearch() throws Exception {
		companyLearnersService.prepareNewLearnershipDevelopmentRegistrationDocs(ConfigDocProcessEnum.LearnerRegistrationByNonMersetaCompanies, companylearners, companylearners, null);
		resolveAddresses();
		checkRequireGaurdian();
		loadUserLanguage();
		loadUsersDisability();
	...
	public void continueRegistration() {
		try {
			...
			if(this.interventionType.getForSdpAccreditaion()) {
				showInfo = true;
			...
			if(this.interventionType.getInterventionTypeEnum() == InterventionTypeEnum.Learnership) {
				this.companylearners.setQualification(this.companylearners.getLearnership().getQualification());
			}else if(SKILLS_PROGRAM_LIST.contains(this.interventionType.getId())) {
				this.companylearners.setQualification(this.selectedSkillsProgram.getQualification());
			}else if(SKILLS_SET_LIST.contains(this.interventionType.getId())) {
				this.companylearners.setQualification(this.selectedSkillsSet.getQualification());
			}else {
				this.companylearners.setQualification(this.selectedQualification);
			}
			
			if(interventionType.getId() == 48) {
	...
	private void validateSdpAccreditation() throws Exception {
		...
		if (this.trainingProvider.getId() != null) {
			if (companyQualificationsService.countfindByCompany(this.trainingProvider, qualification, true) < 1) {				
				...
				throw new Exception("The company selected is not accredited for " + qualification.getDescription());
			} else {
				showInfo = true;			
			}
	...
	public void prepPreferredTrainingProvider() {
		try {
			...
			//this.trainingProvider = selectedCompanyQualification.getCompany();
