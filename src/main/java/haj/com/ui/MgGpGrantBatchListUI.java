...
@ManagedBean(name = "mgGpGrantBatchListUI")
@ViewScoped
public class MgGpGrantBatchListUI extends AbstractUI {
	...
	/* Vars */
	private Double runningTotalForPaymentsReturns = 0.0;
	private Double runningTotalForPaymentsInvoices = 0.0;
	...
	private void runInit() throws Exception {
		if (getSessionUI().getTask() != null && (getSessionUI().getTask().getWorkflowProcess() == ConfigDocProcessEnum.MG_DG_TRANSACTIONS || getSessionUI().getTask().getWorkflowProcess() == ConfigDocProcessEnum.DG_TRANSACTIONS)) {
			...
			if (Boolean.TRUE.equals(getSessionUI().getTask().getFirstInProcess())) {
	...
	private void populateRunningTotals() throws Exception{
		runningTotalForPaymentsReturns = 0.0;
		runningTotalForPaymentsInvoices = 0.0;
...
//			runningTotalForPayments = gpGrantBatchEntryService.sumMandatoryLevyAllEntriesByBatchListToBeRemoved(gpgrantbatchlist.getId(), false);
			// verison two
			runningTotalForPaymentsReturns = gpGrantBatchEntryService.sumMandatoryLevyAllEntriesByBatchListToBeRemovedAndTypeWithRounding(gpgrantbatchlist.getId(), false, GpDocumentType.Return);
			runningTotalForPaymentsInvoices = gpGrantBatchEntryService.sumMandatoryLevyAllEntriesByBatchListToBeRemovedAndTypeWithRounding(gpgrantbatchlist.getId(), false, GpDocumentType.Invoice);
		} else if (gpgrantbatchlist != null && gpgrantbatchlist.getWspType() == WspTypeEnum.Discretionary) {
			runningTotalForPaymentsReturns = gpGrantBatchEntryService.runningTotalForPaymentsDGWithRounding(gpgrantbatchlist.getId(), false);
		}
	}
	...
	public void runValidiationCheck() {
		try {
			service.runValidiationMandatoryGrantTransaction(getSessionUI().getActiveUser(), gpgrantbatchlist);
			getSessionUI().setTask(null);
			ajaxRedirectToDashboard();
		} catch (Exception e) {
			addErrorMessage(e.getMessage() , e);
	...
	public void reopenValidiationCheck() {
		try {
			service.reopenValidiationRunMandatoryGrantTransaction(getSessionUI().getActiveUser(), gpgrantbatchlist);
			determineFinalApproveVisible();
			addInfoMessage("Opened For Alteration");
		} catch (Exception e) {
			addErrorMessage(e.getMessage() , e);
		}
	}
	...
	public Double getRunningTotalForPaymentsInvoices() {
		return runningTotalForPaymentsInvoices;
	}

	public void setRunningTotalForPaymentsInvoices(Double runningTotalForPaymentsInvoices) {
		this.runningTotalForPaymentsInvoices = runningTotalForPaymentsInvoices;
	}

	public Double getRunningTotalForPaymentsReturns() {
		return runningTotalForPaymentsReturns;
	}

	public void setRunningTotalForPaymentsReturns(Double runningTotalForPaymentsReturns) {
		this.runningTotalForPaymentsReturns = runningTotalForPaymentsReturns;
	}
