 ...
 */
@ManagedBean(name = "qmrLecturerDevelopmentCompletedUI")
@ViewScoped
public class QmrLecturerDevelopmentCompletedUI extends AbstractUI {
	...
	public void generateSummaryReport(){
		try {
			if (selectedFinYear == null) {
				...
				qmrSummaryBeanList = service.generateSummaryReportLecturerDevelopmentCompleted(selectedFinYear);
		...
		try {
			if (selectedQuarter == null) {
				...
				if (getNow().before(selectedQuarter.getFromDate())) {
					...
					rawData = null;
					// summary report
					qmrQuarterCountBeanList = service.generateCountReportLecturerDevelopmentCompleted(selectedQuarter);
					//commented by vh
					  if (selectedQuarter.getDataGenerated() != null && selectedQuarter.getDataGenerated()) {
						//savedData = service.fetchSavedDataLecturerDevelopmentCompletedForQuarter(selectedQuarter);
						savedData = service.runLecturerDevelopmentCompletedForQuarterNotSaved(selectedQuarter);
					} else {
						rawData = service.runLecturerDevelopmentCompletedForQuarterNotSaved(selectedQuarter);
					}
					//rawData = service.runLecturerDevelopmentCompletedForQuarterNotSaved(selectedQuarter);
