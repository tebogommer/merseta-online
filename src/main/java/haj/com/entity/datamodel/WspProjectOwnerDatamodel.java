...
public class WspProjectOwnerDatamodel extends LazyDataModel<Wsp> {
	...
	public WspProjectOwnerDatamodel() {
		super();
	}

	public WspProjectOwnerDatamodel(Users users) {
		super();
		this.users = users;
	}
	...
	public List<Wsp> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
		try {
			...
			retorno = wspService.allWspByStatusProjectOwner(first, pageSize, sortField, sortOrder, filters);
			setRowCount(wspService.countWspByStatusProjectOwner(filters));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return retorno;
