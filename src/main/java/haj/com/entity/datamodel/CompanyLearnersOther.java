...
public class CompanyLearnersOther extends LazyDataModel<CompanyLearners> {
	...
	public CompanyLearnersOther(WorkplaceMonitoring workplacemonitoring) {
		super();
	...
	public List<CompanyLearners> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
		...
		try {
			retorno = companyLearnersService.sortAndFilterNotDesignatedOrLearnership(first, pageSize, sortField, sortOrder, filters, workplacemonitoring.getCompany(), InterventionTypeEnum.Learnership);
			setRowCount(companyLearnersService.countNotDesignatedTradeAndLearnership(filters, workplacemonitoring.getCompany(), InterventionTypeEnum.Learnership));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return retorno;
