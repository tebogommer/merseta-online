...
def test_theme_toggle():
    ...
    with sync_playwright() as p:
        ...
        # Check initial Light Mode state
        initial_chip = page.locator("text=☀️ Light Mode")
        assert initial_chip.is_visible(), "Initial state should be Light Mode"
        print("  ✓ Light Mode loaded successfully with initial '☀️ Light Mode' chip")
        ...
        assert toggle_button.is_visible(), "Toggle dark mode button should be visible in AppBar"
        toggle_button.click()
        time.sleep(1)

        # Verify Dark Mode activated
        dark_chip = page.locator("text=🌙 Dark Mode")
        assert dark_chip.is_visible(), "Chip should now display '🌙 Dark Mode'"
        
        # Check if MudBlazor applied dark theme classes
        has_dark_mode = page.evaluate("() => document.body.classList.contains('mud-theme-dark') || document.querySelector('.mud-layout.mud-theme-dark') !== null || document.querySelector('.mud-theme-dark') !== null")
        ...
        time.sleep(1)
        tasks_heading = page.locator("h1:has-text('Universal Task Inbox')")
        ...
        time.sleep(1)
        employer_title = page.locator("text=Employer & Organisation Registry")
        ...
        toggle_button.click()
        time.sleep(1)
        light_chip_again = page.locator("text=☀️ Light Mode")
        assert light_chip_again.is_visible(), "Chip should revert to '☀️ Light Mode'"
